...
export const errorHandler = (error, ctx) => {
    ...
    return config.isPackage || ctx.req.query('format') === 'json'
        ...
        : ctx.html(_jsx(Error, { requestPath: requestPath, message: message, errorRoute: hasMatchedRoute ? matchedRoute : requestPath, nodeVersion: process.version }));
};
export const notFoundHandler = (ctx) => errorHandler(new NotFoundError(), ctx);
