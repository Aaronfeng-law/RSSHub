...
const middleware = async (ctx, next) => {
    ...
    if (data) {
        ...
        if (ctx.req.query('sorted') !== 'false') {
            data.item = data.item.toSorted((a, b) => +new Date(b.pubDate || 0) - +new Date(a.pubDate || 0));
        }
        ...
        if (ctx.req.query('filter')) {
            ...
            data.item = data.item.filter((item) => {
                ...
                const isFilter = regex instanceof RE2JS
        ...
        if (ctx.req.query('mode')?.toLowerCase() === 'fulltext') {
            const tasks = data.item.map(async (item) => {
                ...
                const parsed_result = await cache.tryGet(`mercury-cache-${link}`, async () => {
                    if (link) {
        ...
        if (ctx.req.query('chatgpt') && config.openai.apiKey) {
            data.item = await Promise.all(data.item.map(async (item) => {
                try {
                    // handle description
