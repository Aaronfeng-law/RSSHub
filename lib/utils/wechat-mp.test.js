...
`;
const testFetchArticleFinishArticleItem = async (path, { setMpNameAsAuthor = false, skipLink = false } = {}) => {
...
describe('wechat-mp', () => {
    it('ExtractMetadata.common', () => {
        ...
        expect(ExtractMetadata.common(load(genScriptHtmlStr(`
    ...
    it('ExtractMetadata.img', () => {
        ...
        expect(ExtractMetadata.img(load(genScriptHtmlStr(`
    ...
    it('ExtractMetadata.audio', () => {
        ...
        expect(ExtractMetadata.audio(load(genScriptHtmlStr(`
            reportOpt = {
              voiceid: "",
              uin: "",
              biz: "",
    ...
    it('ExtractMetadata.location', () => {
        ...
        expect(ExtractMetadata.location(load(genScriptHtmlStr(`
            window.ip_wording = {
              countryName: '中国',
              countryId: '156',
              provinceName: '广东',
    ...
    it('fixArticleContent', () => {
        ...
        const divFooter = '</div>';
        const codeSection = '<section class="code-snippet__fix code-snippet__js">' +
            ...
            '</pre></section>';
        const expectedCodeSection = '<p class="code-snippet__fix code-snippet__js">' +
            '<pre class="code-snippet__js">' +
            '<code><span class="code-snippet_outer">Line1 {</span></code>' +
            '<br>' +
            '<code><span class="code-snippet__keyword">Line2</span></code>' +
        ...
        const expectedHtmlSection = expectedCodeSection + '<p>test</p>' + '<div><p>test</p></div>' + '<div><div>test</div></div>' + '<div><div><p>test</p></div></div>' + '<div><div><p>test</p></div></div>' + '<p>test</p>' + '<div><p>test</p></div>';
    ...
    it('fetchArticle_&_finishArticleItem_appMsg', async () => {
        ...
        expect($('iframe').attr()).toMatchObject({
            src: 'https://v.qq.com/txp/iframe/player.html?origin=https%3A%2F%2Fmp.weixin.qq.com' +
    ...
    it('deleted_page', async () => {
        ...
        try {
            ...
            expect(error.message).not.toContain('.style');
            expect(error.message).not.toContain('Consider raise an issue');
            expect(error.message).toContain('deleted by author:');
            expect(error.message).toContain('Title 该内容已被发布者删除');
            expect(error.message).toContain(deletedPageUrl);
        }
    });
    it('redirect', () => {
...
//# sourceMappingURL=wechat-mp.test.js.map