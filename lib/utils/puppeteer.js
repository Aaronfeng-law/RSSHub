...
const outPuppeteer = async () => {
    ...
    const insidePuppeteer = puppeteer;
    ...
    const browser = await (config.puppeteerWSEndpoint
        ...
        : insidePuppeteer.launch(config.chromiumExecutablePath
            ? {
 ...
 */
export const getPuppeteerPage = async (url, instanceOptions = {}) => {
    const options = {
        args: [
            '--no-sandbox',
            '--disable-setuid-sandbox',
    ...
    if (config.puppeteerWSEndpoint) {
        ...
        browser = await insidePuppeteer.launch(config.chromiumExecutablePath
            ? {
                executablePath: config.chromiumExecutablePath,
                ...options,
            }
